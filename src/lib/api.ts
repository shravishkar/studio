...
export async function deleteTask(tenantId: string, token: string, projectId: string, taskId: string): Promise<ApiAddResponse> {
    const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
    if (!baseUrl) {
        throw new Error("API base URL is not configured.");
    }
    ...
    try {
        ...
        const responseData: ApiAddResponse = await response.json();
        if (!responseData.success) {
            throw new Error(responseData.message || "API returned a non-successful response for deleting a task.");
        }

        return responseData;
    } catch (error) {
        console.error("Error deleting task:", error);
        throw error instanceof Error ? error : new Error("An unknown error occurred while deleting the task.");
    }
}
